...
$$
J_\mu(x) = \operatorname{E}_{X_1, \dots, X_n} \left[ g_n(x_n) + \sum_{i = 0}^{n - 1} g_i(x_i, \mu(x_i), x_{i+1}) \Bigg| x_{i} = z \right]
$$
...
$$
J_{\mu}(s) = \operatorname{E} \left[ \sum_{t = 0}^{+\infty} \gamma^t r(s_t, a_t) \Big| s_0 = s \right]
$$
...
Bellman equation
$$
J_\mu(s) = \operatorname{E}_{a, s'} \left[ r(s, a) + \gamma J_\mu(s') \mid s_0 = s \right]
$$
...
- Law of large numbers (LLN)
  ...
  - For computing $\operatorname{E}(X)$

  - If we have access to i.i.d. samples $X_1, X_2, \dots, X_n$

  - Then LLN says that
    $$
    \frac{X_1 + \dots X_n}{n} \to \operatorname{E}(X)
    $$
...
- Is such incremental versions possible to find everytime?
  ...
  $$
  f(x) = \frac{1}{2} (x - \operatorname{E}(x))^2
  $$

  $$
  \nabla f(x) = x - \operatorname{E} X
  $$

  $$
  x_{n+1} = x_n + \alpha_n [- \nabla f(x_n)] = x_n + \alpha_n (\operatorname{E}X - x_n)
  $$
...
If we allow a Markov chain to evlove, the MC goes to it's stationary distribution.
